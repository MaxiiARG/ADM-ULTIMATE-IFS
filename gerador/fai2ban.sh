...
[[ "$apache" != "" ]] && s4="apache"
echo -e "${cor[5]} $(fun_trans "Este y Fail2ban Protection")"
echo -e "${cor[5]} $(fun_trans "Optimo para proteger contra ataques de DDos")"
echo -e "$barra"
if [[ ! -z "$pid_fail" ]]; then
 ...
 echo -e "${barra}"
  while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Selecione una Opcion"): " && read logxyz
   tput cuu1 && tput dl1
  done
 case ${logxyz} in
...
echo -e "${cor[5]} $(fun_trans "¿Quieres Instalar Fail2ban??")"
echo -e "$barra"
  while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
   echo -ne "\033[1;37m$(fun_trans "Seleccione la opción") [S/N]: " && read fail2ban
   tput cuu1 && tput dl1
  done
...
echo -e "${cor[5]} $(fun_trans "Fail2ban será activo en los Siguientes Puertos y Servicos"):"
echo -e "${barra}"
[ "$s1" != "" ] && echo -ne " $s1"
[ "$s2" != "" ] && echo -ne " $s2"
[ "$s3" != "" ] && echo -ne " $s3"
